...
func TestUser(t *testing.T) {
	t.Run("create an user", func(t *testing.T) {
		...
		t.Run("invalid json", func(t *testing.T) {
			t.Parallel()
			rec := doRequest(t, "POST", "/api/users", `{"name":"test","email":`)
			assert(t, 400, rec.Code)
		})

		t.Run("invalid request body", func(t *testing.T) {
			t.Parallel()
			rec := doRequest(t, "POST", "/api/users", `{"email":"test2@example.com"}`)
			assert(t, 400, rec.Code)

			rec = doRequest(t, "POST", "/api/users", `{"name":"test2"}`)
			assert(t, 400, rec.Code)

			rec = doRequest(t, "POST", "/api/users", `{"name":"test2","email":"not_email"}`)
			assert(t, 400, rec.Code)
		})
	})
	...
	t.Run("get an user", func(t *testing.T) {
		t.Run("success: user1", func(t *testing.T) {
			t.Parallel()
			rec := doRequest(t, "GET", "/api/users/"+userIDMap["user1"].String(), "")
			assert(t, 200, rec.Code)
